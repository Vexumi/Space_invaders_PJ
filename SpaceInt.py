...
"""
Type of Enemy - SpaceInt, damage 10, self hp = 10 - 30
"""


class SpaceInt(pygame.sprite.Sprite):
    ...
    def __init__(self, game, all_sprites):
        ...
        self.shoot_delay = 1
        self.start_shoot_delay = random.randint(0, 3)
        self.type_of_alien = self.type_of_aliens[random.randint(0, 1)]
        self.alien_id = random.randint(1, 1000)

        self.init_alien()

    def init_alien(self):
        schedule.every(self.start_shoot_delay).seconds.do(self.start_shoot).tag(f'start_shoot {self.alien_id}')
        self.alien_speed = random.randint(3, 4)

        # создание спрайта персонажа
        self.image = load_image('SpaceInt.png', (255, 255, 255))
