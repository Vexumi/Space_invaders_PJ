...
class Nova(pygame.sprite.Sprite):
    ...
    def update(self, event):
        ...
        if pressed[pygame.K_LSHIFT]:  # activate shield
            sprites = [type(sprite) for sprite in self.game.all_sprites]
            if Shield not in sprites:
                self.shield_on = Shield(self, self.shield)
        else:  # deactivate shield
            self.shield_on = False
            for sprite in self.game.all_sprites:
                if type(sprite) == Shield:
    ...
    def change_image(self):
        if not self.direction:
            ...
            self.image = load_image('Nova_left.png', (255, 255, 255))
            self.image = pygame.transform.scale(self.image, (80, 80))
        elif self.direction == 'Right':
            self.image = load_image('Nova_right.png', (255, 255, 255))
            self.image = pygame.transform.scale(self.image, (80, 80))
    ...
    def check_collision(self):
        ...
        if bullet_collided and not self.shield_on:
